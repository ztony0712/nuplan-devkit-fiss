...
class EgoAgentEncoder(nn.Module):
    def __init__(self):
        super(EgoAgentEncoder, self).__init__()
...
class CrossTransformer(nn.Module):
    ...
    def forward(self, query, key, value, mask=None): #torch.Size([8, 1, 256]) torch.Size([8, 100, 256])
...
class Agent2Map_MDF(nn.Module): #MDF
    def __init__(self):
        super(Agent2Map_MDF, self).__init__()
        self.lane_attention = CrossTransformer()
        # self.crosswalk_attention = CrossTransformer()
        self.map_attention = MultiModalTransformer() 

    def forward(self, actor, lanes, mask):
        query = actor.unsqueeze(1) #actor.shape:torch.Size([8, 256]) -> query.shape: torch.Size([8, 1, 256])
        lanes_actor = [self.lane_attention(query, lanes[:, i], lanes[:, i]) for i in range(lanes.shape[1])] #[torch.Size([8, 1, 256])]
        map_actor = torch.cat(lanes_actor, dim=1) #torch.Size([8, 6, 256])
        # map_actor = map_actor.squeeze(2) # torch.Size([6, 8, 256])
        # map_actor = torch.stack(lanes_actor,dim=1) #
        output = self.map_attention(query, map_actor, map_actor, mask).squeeze(2)
...
class AVDecoder(nn.Module):
    def __init__(self, future_steps=50, feature_len=9):
        ...
        self.control = nn.Sequential(nn.Dropout(0.1), nn.Linear(512, 256), nn.ELU(), nn.Linear(256, future_steps))
    ...
    def forward(self, agent_map, agent_agent):
        ...
        # actions = self.control(feature).view(-1, 3, self._future_steps, 2) #feature:([8, 3, 512])
        feature = torch.cat([agent_map, agent_agent.unsqueeze(1).repeat(1, 3, 1)], dim=-1) #feature:([8, 3, 512])
        actions = self.control(feature).view(-1, 1, self._future_steps, 3) 
...
class Predictor(nn.Module):
    def __init__(self, future_steps):
        ...
        self.agent_map_MDF = Agent2Map_MDF() # MDF
    ...
    def forward(self, ego, neighbors, lane_features):
        # actors
        ego_actor = self.ego_net(ego) #8*5*7 -> torch.Size([8, 256])
        vehicles = torch.stack([self.vehicle_net(neighbors[:, i]) for i in range(10)], dim=1) #8*10*5*8 -> torch.Size([8, 10, 256])
        pedestrians = torch.stack([self.pedestrian_net(neighbors[:, i]) for i in range(10)], dim=1)  #torch.Size([8, 10, 256])
        cyclists = torch.stack([self.cyclist_net(neighbors[:, i]) for i in range(10)], dim=1) # torch.Size([8, 10, 256])
        neighbor_actors = torch.where(neighbors[:, :, -1, -1].unsqueeze(2)==2, pedestrians, vehicles) # torch.Size([8, 10, 256])
        neighbor_actors = torch.where(neighbors[:, :, -1, -1].unsqueeze(2)==3, cyclists, neighbor_actors) # torch.Size([8, 10, 256])
        actors = torch.cat([ego_actor.unsqueeze(1), neighbor_actors], dim=1) # torch.Size([8, 11, 256])
        actor_mask = torch.eq(torch.cat([ego.unsqueeze(1), neighbors[..., :-1]], dim=1), 0)[:, :, -1, -1] # torch.Size([8, 11])

        # maps
        # lane_feature = map_lanes # torch.Size([8, 11, 6, 100, 256])
        # crosswalk_feature = self.crosswalk_net(map_crosswalks)
        lane_mask = torch.eq(lane_features, 0)[:, :, :, 0, 0] #torch.Size([8, 11, 6])
        # crosswalk_mask = torch.eq(map_crosswalks, 0)[:, :, :, 0, 0]
        # map_mask = torch.cat([lane_mask, crosswalk_mask], dim=2)
        map_mask = lane_mask
        map_mask[:, :, 0] = False # prevent nan # torch.Size([8, 11, 6])
        ...
        for i in range(actors.shape[1]):
            output = self.agent_map_MDF(agent_agent[:, i], lane_feature[:, i], map_mask[:, i]) # MDF 
