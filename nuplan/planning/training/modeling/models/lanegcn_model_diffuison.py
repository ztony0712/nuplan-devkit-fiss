...
class LaneGCN(TorchModuleWrapper):
    ...
    def __init__(
        ...
        self.ego_input_dim = (past_trajectory_sampling.num_poses + 1) * Agents.ego_state_dim() # (4+1)*3 = 15 
        self.agent_input_dim = (past_trajectory_sampling.num_poses + 1) * Agents.agents_states_dim() # (4+1)*8 = 40
    ...
    def forward(self, features: FeaturesType) -> TargetsType:
        ...
        ego_agent_features = cast(Agents, features["agents"]) #NOTES: agents input
        ego_past_trajectory = ego_agent_features.ego  # batch_size x num_frames x 3 = 8*5*3
        ...
        predictions = self._mlp(ego_features).reshape(batch_size, -1, 2) #NOTES:output
