...
class DataProcess(object):
    ...
    def ego_process(self, sdc_id, timestep, tracks):
        ...
        sdc_states = tracks[sdc_id].states[timestep+1-self.hist_len:timestep+1] # get the sdc states len=20
    ...
    def process_data(self, save_path, viz=True):
        for data_file in self.data_files:
            ...
            for data in dataset:
                ...
                sdc_id = parsed_data.sdc_track_index # sdc_id: self-driving car id 82
                time_len = len(parsed_data.tracks[sdc_id].states) #91
                self.build_map(parsed_data.map_features, parsed_data.dynamic_map_states)

                for timestep in range(self.hist_len, time_len-self.future_len, 10): #(20, 91-50, 10)
                    # process data
...
if __name__ == "__main__":
    # Arguments
    datatype= 'Val' #Train, Val, Test
    parser = argparse.ArgumentParser(description='Data Processing')
    parser.add_argument('--load_path', type=str, help='path to dataset files', default=f'/media/arc/Nuclear/Waymo/{datatype}')
    parser.add_argument('--save_path', type=str, help='path to save processed data', default=f'/media/arc/Nuclear/Waymo_processed/Waymo_{datatype}')
