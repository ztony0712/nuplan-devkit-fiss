...
def main(cfg: DictConfig) -> Optional[TrainingEngine]:
    ...
    build_training_experiment_folder(cfg=cfg) #/tmp/tutorial_nuplan_framework/training_DIPP_experiment/
...
if __name__ == '__main__':
    ...
    EXPERIMENT = 'training_DIPP_experiment'#'training_raster_experiment'
    ...
    cfg = hydra.compose(config_name=CONFIG_NAME, overrides=[
        ...
        f'cache.cache_path={str(SAVE_DIR)}/cache', # /tmp/tutorial_nuplan_framework/cache #NOTES: default_training
        f'experiment_name={EXPERIMENT}', #NOTES: default_experiment
        'py_func=train', #NOTES: default_training
        '+training=training_urban_driver_open_loop_model',  # raster model that consumes ego, agents and map raster layers and regresses the ego's trajectory #NOTES: nuplan/planning/script/experiments/training
        ...
        'data_loader.params.num_workers=2',#NOTES: [in nuplan/planning/script/config/training/data_loader/default_data_loader.yaml]
        # 'splitter=nuplan'
    ])
